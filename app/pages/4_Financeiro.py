...
.op-card { border-radius: 10px; padding: 14px 16px; color: #fff;
  box-shadow: 0 6px 16px rgba(0,0,0,.12); border: 1px solid rgba(255,255,255,.12);
...
.op-panel { border: 1px solid rgba(0,0,0,.08); border-radius: 10px;
  padding: 12px 12px 10px 12px; background: rgba(255,255,255,.55); }
...
.op-mainline { font-size: 14px; font-weight: 700; white-space: nowrap; overflow: hidden;
  text-overflow: ellipsis; max-width: 520px; }
.op-subline { font-size: 12px; opacity: .75; white-space: nowrap; overflow: hidden;
  text-overflow: ellipsis; max-width: 520px; }
...
if df_month_full.empty:
    ...
    dfc["category_name"] = dfc.get("category_name", "").fillna("").apply(_clean_str)
...
def _render_list_panel(df_in: pd.DataFrame, empty_text: str):
    ...
    for col in ["description", "counterparty_name", "project_code", "status"]:
        if col in dfp.columns:
            dfp[col] = dfp[col].fillna("").apply(_clean_str)
