...
# ===== STATUS: separa "status da data" x "status interno" =====
# status da data (o que deve APARECER na barra)
# (na view, geralmente é date_confidence)
df["status_date"] = df.get("date_confidence", "").fillna("")
df["status_date_norm"] = df["status_date"].apply(normalize_status)

# status interno (usado apenas pra CANCELADA / filtro / cor cinza)
df["status_internal"] = df.get("status", "").fillna("")
df["status_internal_norm"] = df["status_internal"].apply(normalize_status)
...
# Atalhos de período (1, 2, 3 meses e mês anterior+atual)
...
# canceladas (usa status INTERNO)
if not show_cancelled and "status_internal_norm" in f.columns:
    f = f[f["status_internal_norm"] != "CANCELADA"]
...
# Texto dentro da barra (usa status DA DATA)
status_icon = {
    "CONFIRMADO": "✓",
    "PLANEJADO": "",
    "ESTIMADO": "≈",
...
if show_status:
    f["bar_text"] = f.apply(
        lambda r: (
            (status_icon.get(safe_text(r.get("status_date_norm")), "") + " " + safe_text(r.get("status_date_norm")) + " - " + safe_text(r.get("assignee_names")))
            .strip()
        ),
        axis=1,
    )
else:
    # hint leve para canceladas (pelo status interno)
    f["bar_text"] = f.apply(
        lambda r: (("✖ " if safe_text(r.get("status_internal_norm")) == "CANCELADA" else "") + safe_text(r.get("assignee_names"))).strip(),
        axis=1,
    )

# Cor: admin diferente + cancelada cinza (se estiver visível)
f["tipo_plot"] = f["tipo_atividade"].astype(str)
if "status_internal_norm" in f.columns:
    f.loc[f["status_internal_norm"] == "CANCELADA", "tipo_plot"] = "CANCELADA"
...
fig = px.timeline(
    ...
    hover_data={
        ...
        "assignee_names": True,
        "tipo_atividade": True,
        "date_confidence": True,     # status da data
        "status": True,              # status interno
        "start_date": True,
        "end_date": True,
        "label": False,
        "plot_start": False,
        "plot_end": False,
        "tipo_plot": False,
        "status_date_norm": False,
        "status_internal_norm": False,
    },
)
