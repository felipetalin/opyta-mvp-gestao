...
try:
    ...
    dash_df = df_f.copy()
    dash_df["expense_month"] = pd.to_datetime(dash_df["expense_date"], errors="coerce").dt.to_period("M").dt.to_timestamp()
    dash_df["payment_month"] = pd.to_datetime(dash_df["payment_date"], errors="coerce").dt.to_period("M").dt.to_timestamp()

    min_month = _month_start(date_from)
    max_month = _month_start(date_to)
    months = pd.date_range(pd.to_datetime(min_month), pd.to_datetime(max_month), freq="MS")
    monthly = pd.DataFrame({"month": months})

    launched = (
        dash_df[dash_df["status"] != "GLOSADO"]
        .groupby("expense_month")["amount"]
        .sum()
        .rename("Lancadas")
    )
    paid = (
        dash_df[dash_df["status"] == "PAGO"]
        .groupby("payment_month")["amount"]
        .sum()
        .rename("Pagas")
    )
    pending = (
    ...
    c3, c4 = st.columns(2)
    with c3:
        st.caption("Distribuicao das despesas por categoria")
        cat_df = (
            dash_df[dash_df["status"] != "GLOSADO"]
            ...
            .sort_values(ascending=False)
            .reset_index()
            .rename(columns={"category_name": "Categoria", "amount": "Valor"})
        )
        if cat_df.empty:
            st.caption("Sem dados.")
        else:
            fig = px.pie(cat_df, names="Categoria", values="Valor", hole=0.48)
    ...
    with c4:
        ...
        if aging.empty:
            st.caption("Sem pendencias.")
        else:
            aging["Faixa"] = pd.cut(aging["days_open"], bins=bins, labels=labels)
            aging_df = (
                aging.groupby("Faixa", observed=False)["amount"]
                .sum()
                .reindex(labels)
                .fillna(0.0)
                .reset_index()
                .rename(columns={"amount": "Valor"})
            )
            fig = px.bar(aging_df, x="Faixa", y="Valor")
            fig.update_layout(height=330, margin=dict(l=10, r=10, t=10, b=10), xaxis_title="", yaxis_title="R$")
            st.plotly_chart(fig, use_container_width=True)
except Exception as e:
    st.warning("Nao foi possivel montar os dashboards.")
    st.code(_api_error_message(e))
